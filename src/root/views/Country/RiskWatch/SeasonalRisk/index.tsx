...
function SeasonalRisk(props: Props) {
  ...
  const { response: gwisResponse } = useRequest<ListResponse<GWIS>>({
    skip: !country,
    query: {
    ...
    url: 'risk://api/v1/gwis',
  });

  const gwisChartData = useMemo(() => {
    if (isNotDefined(gwisResponse)) {
      return [];
    }
    const grouped = listToGroupList(gwisResponse?.results, (gwis) => gwis.month);

    const anotherGrouped = mapToMap(
      grouped,
      (key) => key,
      (val) => {
        ...
        return {
          ...
          dsr_min: sum(val.map((v) => v.dsr_min)) / val.length,
    ...
    return mapToList(anotherGrouped);
  }, [gwisResponse]);
