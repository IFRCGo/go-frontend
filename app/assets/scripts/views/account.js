...
import { FormCheckboxGroup, FormInput } from '../components/form-elements/';
...
class Account extends React.Component {
  constructor (props) {
    ...
    this.createRegionGroupedDocumentData = this.createRegionGroupedDocumentData.bind(
      this
    );
  ...
  componentDidMount () {
    ...
    const {
      user,
      _getProfile,
      _getFieldReportsByUser,
      _getPerCountries,
      _getPerDocuments,
      _getPerDraftDocument
    } = this.props;
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.profile.receivedAt !== nextProps.profile.receivedAt) {
      if (
        typeof nextProps.profile.data !== 'undefined' &&
        nextProps.profile.data !== null &&
        typeof nextProps.profile.data.subscription !== 'undefined' &&
        nextProps.profile.data.subscription !== null
      ) {
        nextProps.profile.data.subscription.forEach(subscription => {
          if (
            typeof subscription.event !== 'undefined' &&
    ...
    if (this.props.profile.fetching && !nextProps.profile.fetching) {
      ...
      if (nextProps.profile.error) {
        showAlert(
          'danger',
          <p>
            <strong>Error:</strong> Could not load user profile
          </p>,
          true,
          4500
        );
    ...
    if (this.props.profile.updating && !nextProps.profile.updating) {
      ...
      if (nextProps.profile.updateError) {
        showAlert(
          'danger',
          <p>
            <strong>Error:</strong> {nextProps.profile.updateError.detail}
          </p>,
          true,
          4500
        );
    ...
    if (
      nextProps.perForm.getPerCountries.fetched &&
      nextProps.perForm.getPerCountries.data.count > 0
    ) {
      this.setState({
        chosenCountry: {
          id: nextProps.perForm.getPerCountries.data.results[0].id,
          society_name:
            nextProps.perForm.getPerCountries.data.results[0].society_name
        }
      });
    }
  }
  ...
  syncNotificationState (data) {
    ...
    subscriptions.forEach(sub => {
      ...
      if (rtype === 'country' && sub.country) {
        ...
        next.countries = next.countries.concat([
          { label: countryMeta.label, value: sub.country.toString() }
        ]);
        ...
        next.disasterTypes = updateChecks(
          next.disasterTypes,
          sub.dtype.toString()
        );
      } else if (
  ...
  serializeNotifications (notifications) {
    ...
    let surgeNotifications = get(notifications, 'surg', [])
      .filter(d => d.checked)
      .map(d => ({
        type: d.value,
        value: true
    ...
    let perDueDates = get(notifications, 'per', [])
      .filter(d => d.checked)
      .map(d => ({
        type: d.value,
        value: true
    ...
    let basicNotifications = get(notifications, 'basic', [])
      .filter(d => d.checked)
      .map(d => ({
        type: d.value,
        value: true
  ...
  onProfileSubmit (e) {
    ...
    showGlobalLoading();
    const id = this.props.profile.data.id;
    this.props._updateProfile(
      id,
      this.serializeProfile(
        profileAttributes.slice(1, profileAttributes.length)
      )
    );
  }
  ...
  serializeProfile (attributes) {
    ...
    attributes.forEach(d => {
      ...
      nextValue =
        nextValue && nextValue.hasOwnProperty('value')
          ? nextValue.value
          : nextValue;
  ...
  changeChosenCountry (e) {
    let filteredCountry = this.props.perForm.getPerCountries.data.results.filter(
      country => country.id === parseInt(e.target.value)
    );
    this.setState({
      chosenCountry: {
        id: filteredCountry[0].id,
        society_name: filteredCountry[0].society_name
      }
    });
  }
  ...
  isPerPermission () {
    return (
      ...
      (this.props.getPerMission !== 'undefined' &&
          ...
          <dl className="dl--horizontal">
            {profile.data
              ? profileAttributes.map(a => (
                <Fragment key={a[0]}>
                  <dt className="form__label__uppercase">
                    {apiPropertyDisplay(a[0])}
                  </dt>
          ...
          <section className="fold">
            <div className="inner">
                      ...
                      <div className="report__list--header list__each__block flex">
                        <div>
                          ...
                          <div className="report__list--updated global-margin-t">
                            Last Updated: {DateTime.fromISO(o.updated_at || o.created_at).toISODate()}
                          </div>
                        </div>
                      </div>
                      <p>{o.description}</p>
              ...
              <div className="fold__footer">
                <p>
                  To delete a field report, contact <a href="mailto:im@ifrc.org">the IM team</a>.
                </p>
              </div>
            </div>
          </section>
  ...
  renderSubscriptionForm () {
    this.props.profile.data.subscription.filter(
      subscription => subscription.event !== null
    );
    const events = [];
    Object.keys(this.props.event.event).forEach(event => {
      events.push(
        ...
        <div className="fold-container">
          <Fold title="Subscription preferences" foldClass="margin-reset">
            ...
            <FormCheckboxGroup
              ...
              description={
                'Select one or more regions to receive notifications about.'
              }
              name="regions"
              classWrapper="action-checkboxes"
              options={regions}
            ...
            <div className="form__group">
              ...
              <p className="form__description">
                Select one or more countries to receive notifications about.
              </p>
              <Select
                name="countries"
            ...
            <FormCheckboxGroup
              ...
              description={
                'Get notified about new disasters in these categories.'
              }
              name="disasterTypes"
              classWrapper="action-checkboxes"
  ...
  createRegionGroupedDocumentData () {
    ...
    if (this.props.perOverviewForm.fetched) {
      this.props.perOverviewForm.data.results.forEach(perOverviewForm => {
        ...
        if (
          perOverviewForm.country.region === null ||
          perOverviewForm.country.region === ''
        ) {
          perOverviewForm.country.region = -1;
        }
        if (!groupedDocuments.hasOwnProperty(perOverviewForm.country.region)) {
          groupedDocuments[perOverviewForm.country.region] = {
            [perOverviewForm.country.id]: []
          };
          groupedDocuments[perOverviewForm.country.region][perOverviewForm.country.id].push(perOverviewForm);
        } else {
          if (
            !groupedDocuments[perOverviewForm.country.region].hasOwnProperty(
              perOverviewForm.country.id
            )
          ) {
    ...
    if (
      ...
      !!this.props.perForm.getPerDocuments.data &&
      !!this.props.perForm.getPerDocuments.data.results
    ) {
      this.props.perForm.getPerDocuments.data.results.forEach(document => {
        if (document.country !== null) {
          ...
          if (!groupedDocuments.hasOwnProperty(document.country.region)) {
            groupedDocuments[document.country.region] = {
              [document.country.id]: []
            };
            groupedDocuments[document.country.region][document.country.id].push(
              document
            );
          } else {
            if (
              !groupedDocuments[document.country.region].hasOwnProperty(
                document.country.id
              )
            ) {
  ...
  renderPerFormDocuments (documents) {
    ...
    Object.keys(documents).forEach((regionKey, regionIndex) => {
      ...
      Object.keys(documents[regionKey]).forEach((countryKey, countryIndex) => {
        ...
        documents[regionKey][countryKey].forEach(document => {
          ...
          if (document.formType === 'overview') {
            perDocuments.push(
              <React.Fragment key={'documentoverviewrow' + document.id}>
                <div className="list__each__block flex">
                  <div key={'documentov' + document.id}>
                    Overview - {document.date_of_current_capacity_assessment.substring(0, 10)} - {typeof document.user !== 'undefined' && document.user !== null
                  ...
                  <div className="list__each__button">
                    <Link
                      ...
                      to={'/view-per-forms/overview/' + document.id}
                    >
                      View
                    </Link>
                  </div>
            ...
            perDocuments.push(
              <React.Fragment
                key={'documentrow' + document.code + 'id' + document.id}
              >
                <div className="list__each__block flex">
                  <div key={'document' + document.id}>
                    {document.code.toUpperCase()} - {document.name} - {document.updated_at.substring(0, 10)} - {typeof document.user !== 'undefined' &&
                    document.user !== null
                  ...
                  <div className="list__each__button">
                    <Link
                      className="button button--small button--secondary-bounded"
                      to={
                        '/view-per-forms/' + document.code + '/' + document.id
                      }
                    >
                      View
                    </Link>
        ...
        countries.push(
          <div key={'countryDocument' + countryKey}>
            <div className="heading-sub global-spacing-v">
              {currentCountryName}
            </div>
            {perDocuments}
            <br />
      ...
      regions.push(
        <div key={'regionDocument' + regionKey}>
          <span className="fold__title">{getRegionById(regionKey).name}</span>
          {countries}
          <br />
        </div>
      );
  ...
  renderPerFormsComponent () {
    ...
    if (
      ...
      typeof this.props.perForm.getPerCountries.data.results !== 'undefined' &&
      this.props.perForm.getPerCountries.data.results !== null
    ) {
      this.props.perForm.getPerCountries.data.results.forEach(country => {
        const societyName =
          country.society_name !== null && country.society_name.trim() !== ''
            ? country.society_name
            : country.name + ' NS';
        countryOptions.push(
          <option value={country.id} key={'persociety' + country.id}>
            {societyName}
          </option>
        );
      });
    ...
    if (this.props.perForm.getPerDocuments.fetched) {
      documents = this.renderPerFormDocuments(
        this.createRegionGroupedDocumentData()
      );
    }
    return (
      <div className="fold-container">
        <section className="fold" id="per-forms">
          <div className="inner">
            ...
            <hr />
            Click on the following links to access the PER forms, where you can
            select individual National Societies.
            <br />
            ...
            <div className="text-center">
              <Link
                to={'/per-forms/overview/' + this.state.chosenCountry.id}
                className="button button--medium button--secondary-bounded"
              >
                Overview
              </Link>
            ...
            <div className="clearfix">
              <div className="per__form__col">
                <Link
                  to={
                    '/per-forms/policy-strategy/' + this.state.chosenCountry.id
                  }
                  className="button button--medium button--secondary-bounded"
                >
                  Area 1: Policy and Standards
                </Link>
              </div>
              <div className="per__form__col">
                <Link
                  to={
                    '/per-forms/analysis-and-planning/' +
                    this.state.chosenCountry.id
                  }
                  className="button button--medium button--secondary-bounded"
                >
                  Area 2: Analysis and Planning
                </Link>
              </div>
              <div className="per__form__col">
                <Link
                  to={
                    '/per-forms/operational-capacity/' +
                    this.state.chosenCountry.id
                  }
                  className="button button--medium button--secondary-bounded"
                >
                  Area 3: Operation capacity
                </Link>
              </div>
              <div className="per__form__col">
                <Link
                  to={
                    '/per-forms/operational-capacity-2/' +
                    this.state.chosenCountry.id
                  }
                  className="button button--medium button--secondary-bounded"
                >
                  Area 3: Operational capacity 2
                </Link>
              </div>
              <div className="per__form__col">
                <Link
                  to={'/per-forms/coordination/' + this.state.chosenCountry.id}
                  className="button button--medium button--secondary-bounded"
                >
                  Area 4: Coordination
                </Link>
              ...
              <div className="per__form__col">
                <Link
                  to={
                    '/per-forms/operations-support/' +
                    this.state.chosenCountry.id
                  }
                  className="button button--medium button--secondary-bounded"
                >
                  Area 5: Operations support
                </Link>
              </div>
            </div>
            <br />
  ...
  renderDraftDocuments () {
    ...
    if (this.props.perForm.getPerDraftDocument.fetched) {
      let index = 0;
      if (
        typeof this.props.perForm.getPerDraftDocument.data.results !==
          'undefined' &&
        this.props.perForm.getPerDraftDocument.data.results !== null
      ) {
        this.props.perForm.getPerDraftDocument.data.results.forEach(
          draftDocument => {
            let parsedData = null;
            try {
              ...
              console.warn(
                'API provided invalid data for draft document (' +
                  draftDocument.data +
                  ')! renderDraftDocuments () failed!\n\n',
                e
              );
              return;
            }
            draftDocuments.push(
              <div
                className="list__each__block flex"
                key={'draftDocument' + index}
              >
                <div>
                  {draftDocument.code.toUpperCase()} - {parsedData.submitted_at !== ''
                    ? parsedData.submitted_at.substring(0, 10) + ' - '
                    : null} {typeof draftDocument.user !== 'undefined'
                    ? draftDocument.user.username + ' - '
                    : null} {draftDocument.country.name}
                </div>
                <div className="list__each__button">
                  <Link
                    className="button button--small button--secondary-bounded"
                    to={
                      draftDocument.code === 'overview'
                        ? '/per-forms/overview/' + draftDocument.country.id
                        : '/edit-per-forms/' +
                          draftDocument.code +
                          '/' +
                          draftDocument.user.username +
                          '/' +
                          draftDocument.country.id
                    }
                  >
                    Edit
                  </Link>
    ...
    if (Object.keys(this.props.event.event).length > 0) {
      Object.keys(this.props.event.event).forEach(eventId => {
        if (this.props.event.event[eventId].fetched) {
          events.push(
            <div
              key={'operations-component' + eventId}
              className="account__op__block clearfix"
            >
              <div className="account__op__each__link">
                <Link
                  className={'link--primary'}
                  to={'/emergencies/' + eventId}
                >
                  {this.props.event.event[eventId].data.name}
                </Link>
              </div>
        ...
        <section className="fold" id="notifications">
          <div className="inner">
            ...
            <div className="clearfix">
              ...
              <div className="account__op__links">{events}</div>
            </div>
          </div>
        </section>
      </div>
          ...
          >
            ...
            <div className="inpage__body">
              <div className="inner">
                ...
                <TabPanel>
                  <TabContent>{this.renderFieldReports()}</TabContent>
                  <TabContent
                    isError={
                      !this.props.profile.fetched && this.props.profile.error
                    }
                    errorMessage="Subscriptions coming soon"
                    title="Subscriptions"
                  >
                ...
                <TabPanel>
                  <TabContent
                    ...
                    errorMessage="Please login to view content"
                    title="PER Forms"
                  >
