...
import { get, mostRecentReport, dateOptions, datesAgo } from '../utils/utils/';
...
class Emergency extends React.Component {
  ...
  renderFieldReportStats () {
    ...
    const numAffected =
      parseInt(get(report, 'num_affected')) ||
      parseInt(get(report, 'gov_num_affected')) ||
      parseInt(get(report, 'other_num_affected'));
    const numInjured =
      parseInt(get(report, 'num_injured')) ||
      parseInt(get(report, 'gov_num_injured')) ||
      parseInt(get(report, 'other_num_injured'));
    const numDead =
      parseInt(get(report, 'num_dead')) ||
      parseInt(get(report, 'gov_num_dead')) ||
      parseInt(get(report, 'other_num_dead'));
    const numMissing =
      parseInt(get(report, 'num_missing')) ||
      parseInt(get(report, 'gov_num_missing')) ||
      parseInt(get(report, 'other_num_missing'));
    const numDisplaced =
      parseInt(get(report, 'num_displaced')) ||
      parseInt(get(report, 'gov_num_displaced')) ||
      parseInt(get(report, 'other_num_displaced'));
    const numAssisted =
      parseInt(get(report, 'num_assisted')) ||
      parseInt(get(report, 'gov_num_assisted')) ||
      parseInt(get(report, 'other_num_assisted'));
    return (
      <div className="inpage__header-col">
        ...
        <div className="content-list-group">
          <ul className="content-list">
            <li>
              Affected
              <span className="content-highlight">{n(numAffected)}</span>
            </li>
            <li>
              Injured<span className="content-highlight">{n(numInjured)}</span>
            </li>
            <li>
              Dead<span className="content-highlight">{n(numDead)}</span>
            </li>
            <li>
              Missing<span className="content-highlight">{n(numMissing)}</span>
            </li>
            <li>
              Displaced
              <span className="content-highlight">{n(numDisplaced)}</span>
            </li>
          </ul>
          <ul className="content-list">
            <li>
              Assisted
              <span className="content-highlight">{n(numAssisted)}</span>
            </li>
            <li>
              Local staff
              <span className="content-highlight">
                {n(get(report, 'num_localstaff'))}
              </span>
            </li>
            <li>
              Volunteers
              <span className="content-highlight">
                {n(get(report, 'num_volunteers'))}
              </span>
            </li>
            <li>
              Expat delegates
              <span className="content-highlight">
                {n(get(report, 'num_expats_delegates'))}
              </span>
            </li>
          </ul>
        </div>
    ...
    return (
      <div className="inpage__introduction">
        <ul className="introduction-nav">
          <li
            className={c('introduction-nav-item', {
              'introduction-nav-item--active': selected === null
            })}
          >
            <a
              href="#"
              title="Show stats for all appeals"
              onClick={this.onAppealClick.bind(this, null)}
            >
              All Appeals
            </a>
          </li>
          {appeals.map(o => (
            <li
              key={o.id}
              className={c('introduction-nav-item', {
                'introduction-nav-item--active': o.id === selected
              })}
            >
              <a
                href="#"
                title={`Show stats for appeal ${o.name}`}
                onClick={this.onAppealClick.bind(this, o.id)}
              >
                {o.name}
              </a>
            </li>
      ...
      >
        <div>
          <div className="fold__filters">
            ...
            />
            {types.fetched && !types.error ? (
              <FilterHeader
                id="sitrep-type"
                title="Document Type"
                options={[{ value: 'all', label: 'All' }].concat(
                  types.data.results.map(d => ({ value: d.id, label: d.type }))
                )}
                filter={type}
                onSelect={this.handleSitrepFilter.bind(this, 'type')}
              />
            ) : null}
          </div>
              ...
              <div className="inpage__headline-content">
                <div className="inpage__headline-actions">
                  ...
                  <a
                    href={url.resolve(
                      api,
                      `admin/api/event/${data.id}/change/`
                    )}
        ...
        >
          ...
          <div className="inpage__body">
            <div className="inner">
              <TabPanel>
                ...
                >
                  ...
                  >
                    ...
                    {source ? (
                      <p className="emergency__source">Source: {source}</p>
                    ) : null}
                  </Fold>
                </TabContent>
              </TabPanel>
              <TabPanel>
                <TabContent
                  isError={!get(this.props.snippets)}
              ...
              <TabPanel>
                <TabContent
                  ...
                  errorMessage="Field Reports coming soon"
                  title="Field Reports"
                >
              ...
              <TabPanel>
                <TabContent
                  isError={!get(this.props.surgeAlerts, 'data.results.length')}
                  errorMessage="Alerts coming soon"
                  title="Alerts"
                >
                  <SurgeAlertsTable
                    id="alerts"
              ...
              <TabPanel>
                <TabContent
                  isError={!get(this.props.personnel, 'data.results.length')}
                  errorMessage="Personnel coming soon"
                  title="Personnel"
                >
                  <PersonnelTable
                    id="personnel"
              ...
              <TabPanel>
                <TabContent
                  ...
                  errorMessage="Response documents coming soon"
              ...
              <TabPanel>
                <TabContent
                  ...
                  errorMessage="Appeal documents coming soon"
