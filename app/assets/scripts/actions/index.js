...
export function getSitrepsByEventId (id) {
  return fetchJSON(`api/v2/situation_report/?ordering=-created_at&event=${id}&limit=0`, GET_SITREPS, withToken(), { id });
}
...
export function getAppealDocsByAppealIds (appealIds, id) {
  ...
  return fetchJSON(`api/v2/appeal_document/?ordering=-created_at&appeal__in=${ids}`, GET_APPEAL_DOCS, withToken(), { id });
}
